...
const DATA_LAB_GROUPS = ["Renda fixa", "Ações amplas", "Setoriais", "Satélites EUA", "Temas Europeus", "China / China+1", "Hedge"];
...
const dataLabGroupFor = (item) => {
  ...
  if (category.includes("treasuries") || category.includes("treasury bills") || category.includes("inflation usd") || category.includes("credit") || category.includes("high yield") || category.includes("core bond")) return "Renda fixa";
  ...
  if (category.startsWith("core") || category.includes("s&p 500 benchmark") || category.includes("us growth") || category.includes("nasdaq-100")) return "Ações amplas";
  return "Satélites EUA";
};

const DATA_LAB_SUBGROUP_ORDER = {
  "Renda fixa": ["Liquidez USD", "Treasuries por duration", "TIPS / inflação", "Crédito USD", "Core bond global"],
  "Ações amplas": ["EUA amplo", "Growth / Nasdaq", "Core global desenvolvido", "Developed ex-US", "Europa core", "Emergentes"],
...
function DataLabModule() {
  ...
  const [selectedGroup, setSelectedGroup] = useState("Renda fixa");
      ...
      .then(([mainResult, fixedResult]) => {
        ...
        const payload = {
          ...
          instruments: (mainResult.data.instruments || []).map((item) => {
            ...
            const needsFallback = dataLabGroupFor(item) === "Renda fixa" && (!item.performanceChart?.points?.length || item.status !== "ok");
        ...
        setData(payload);
        const firstOk = payload.instruments.find((item) => item.status === "ok" && dataLabGroupFor(item) === "Renda fixa" && item.performanceChart?.points?.length > 1) || payload.instruments.find((item) => item.status === "ok" && item.performanceChart?.points?.length > 1) || payload.instruments?.[0];
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "div",
          null,
          React.createElement("span", { className: "control-title" }, "Fato · Base automatizada"),
      ...
      React.createElement("p", { className: "data-note" }, `Limitação metodológica: ${data?.methodology || "o dashboard usa os dados fixos enquanto o JSON não carrega."}`)
    ),
    React.createElement(
      "section",
      ...
      active
        ? React.createElement(
            ...
            React.createElement(
              "div",
              null,
              React.createElement("span", { className: "control-title" }, "Fato · Instrumento selecionado"),
            ...
            React.createElement("span", { className: "control-title data-lab-calculation-label" }, "Cálculo · Retorno e risco"),
            React.createElement(
              "div",
