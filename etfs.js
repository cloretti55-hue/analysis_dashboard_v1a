...
const CORE_FUNCTIONS = [
  ...
  { fn: "USD Treasuries Acc", why: "A ladder of US Treasury securities by duration.", tickers: ["IB01", "IBTA", "CBU7", "IBTM", "DTLA"], note: "From yield-bearing cash to a directional position on lower rates." },
  { fn: "USD inflation Acc", why: "TIPS with different duration profiles.", tickers: ["TIP5", "ITPS", "ITPE"], note: "Separates short inflation protection from broad inflation exposure." },
  { fn: "Credit and core bonds", why: "Corporate spread exposure and global bond diversification.", tickers: ["LQDA", "IHYA", "AGGU"], note: "Investment grade, high yield and global aggregate bonds serve distinct roles." },
...
const CORE_DETAILS = {
  ...
  EQQQ: ["Structural growth", "Nasdaq-100 in UCITS form.", "Check distribution, cost and trading currency.", ["A European alternative for Nasdaq-100 exposure.", "Useful for separating exposure from vehicle.", "Compare with CNDX according to domicile and distribution policy."]],
  ...
  IMEU: ["European core", "Europe through UCITS.", "Check the benchmark and distribution policy.", ["A regional offshore tool.", "Useful for domicile control.", "Compare with VGK/IEUR/FEZ."]],
  ...
  EXCH: ["Emerging markets ex-China", "EM ex-China in a UCITS wrapper.", "Check liquidity and tracking.", ["Geopolitical separation from EM beta.", "Useful for offshore portfolios.", "Complements VWO/IEMG/EIMI."]],
  ...
  IBTM: ["Intermediate duration", "iShares $ Treasury Bond 7-10yr UCITS ETF USD Acc.", "Partial hedge against falling rates.", ["More sensitive to the monetary-policy cycle.", "Can help when the thesis calls for lower rates.", "Requires more conviction than shorter maturities."]],
  ...
  TIP5: ["Short inflation", "iShares $ TIPS 0-5 UCITS ETF USD Acc.", "Inflation protection with lower duration.", ["Combines positive real rates with lower duration.", "More aligned with real carry than a long-rate bet.", "Avoids mixing inflation exposure with excessive duration."]],
  ITPS: ["Broad inflation", "iShares $ TIPS UCITS ETF Acc.", "US inflation exposure with more duration.", ["Provides realised-inflation exposure through a broader basket.", "Brings greater sensitivity to real rates.", "Can suit portfolios that accept more duration."]],
  ITPE: ["Broad inflation", "iShares $ TIPS UCITS ETF Acc.", "US inflation exposure with more duration.", ["Alternative for broad TIPS exposure.", "The final choice depends on exchange listing and operating policy.", "Compare cost, tracking and trading currency."]],
  ...
  AGGU: ["Global core bonds", "iShares Core Global Aggregate Bond UCITS ETF USD Acc.", "Global bond diversification.", ["Combines sovereign and global credit exposure.", "Can serve as a broad fixed-income building block.", "Currency hedging, duration and composition change its role as a core holding."]],
...
function SatelliteModule() {
  ...
  const [selectedTheme, setSelectedTheme] = useState("Semiconductors / AI hardware");
...
function EuropeModule() {
  ...
  const [selectedTheme, setSelectedTheme] = useState("European defence");
...
function ChinaModule() {
  ...
  const [selectedTheme, setSelectedTheme] = useState("Direct China");
...
function HedgeModule() {
  ...
  const [selectedTheme, setSelectedTheme] = useState("Option income");
...
function CoreModule({ functions = CORE_EQUITY_FUNCTIONS, initialEtf = "VOO", legendMode = "equity", showUcitsToggle = true, dataLabTickers = new Set() } = {}) {
          ...
          ? React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  ...
                  React.createElement("span", { className: "etf-token etf-us" }, "US"),
                  React.createElement("span", { className: "etf-token etf-global" }, "Developed global"),
                  React.createElement("span", { className: "etf-token etf-developed" }, "Developed ex-US"),
                  React.createElement("span", { className: "etf-token etf-emerging" }, "Emerging markets")
                ),
                showUcitsToggle
                  ? React.createElement(
                      "div",
                      { className: "ucits-toggle", "aria-label": "Highlight UCITS ETFs" },
                      React.createElement("button", { type: "button", "aria-pressed": ucitsView === "all", onClick: () => setCoreUcitsView("all") }, "All"),
...
function FixedIncomeModule({ dataLabTickers }) {
  return React.createElement(CoreModule, { functions: FIXED_INCOME_FUNCTIONS, initialEtf: "IB01", legendMode: "fixed", showUcitsToggle: false, dataLabTickers });
}

const SECTOR_GICS_ETFS = [
  { ticker: "XLK", sector: "Information Technology", icon: "chip", tone: "sat-risk-high", role: "Growth / quality", why: "Software, semiconductors and digital infrastructure.", limit: "High concentration and multiples that are more sensitive to growth revisions.", points: ["The most direct technology proxy within the S&P 500.", "Carries a significant share of the index's structural leadership.", "Useful for examining the growth premium versus the benchmark."] },
  { ticker: "XLC", sector: "Communication Services", icon: "phone", tone: "sat-risk-high", role: "Platforms / media", why: "Large digital platforms, media, streaming and telecoms.", limit: "Combines very different businesses and may be concentrated in a few names.", points: ["Useful for separating digital platforms from pure technology.", "Linked to advertising, content and distribution.", "It is not defensive despite including telecoms."] },
...
function SectorGicsModule({ dataLabTickers }) {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement("p", { className: "data-note" }, "Sector proxy: SPDR Select Sector ETFs. Numerical data is available in Data Lab.")
    ),
    React.createElement(
      "aside",
...
function EtfsModule() {
  ...
  const renderActive = () => {
    if (activeEtfTab === "core") return React.createElement(CoreModule, { dataLabTickers });
    if (activeEtfTab === "fixed") return React.createElement(FixedIncomeModule, { dataLabTickers });
    if (activeEtfTab === "sectors") return React.createElement(SectorGicsModule, { dataLabTickers });
