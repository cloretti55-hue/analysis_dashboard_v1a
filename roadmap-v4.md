...
Status: concluída localmente em 2026-08-20.

Entregas implementadas:

- navegação principal reorganizada como Visão geral, Macro, Valuation,
  Instrumentos, Data Lab e Temas;
- rotas existentes preservadas para evitar quebra de links e histórico;
- títulos e descrições alinhados em português no computador e no celular;
- categorias Fixed Income e Core Equities apresentadas como Renda fixa e Ações
  amplas no Data Lab e na área de Instrumentos;
- guia comum separando fato, cálculo, interpretação e limitação em todas as
  áreas analíticas;
- vínculo contextual da área de Instrumentos para as comparações quantitativas
  do Data Lab;
- conclusões prescritivas e formulações de recomendação substituídas por
  descrições de características, relações, riscos e limitações;
- Curvas, Valuation, Instrumentos, Data Lab e Temas revisados para identificar
  a natureza de suas leituras públicas;
- navegação, vínculo contextual e responsividade verificados em computador e
  celular, sem erros de navegador ou rolagem horizontal.
