...
RUNTIME_JS = APP_JS + CURVES_FED_JS + VALUATION_JS + ETFS_JS + DATA_LAB_JS + THEMES_JS + DATA_CLIENT_JS
...
class FrontendDataClientTests(unittest.TestCase):
    ...
    def test_modules_use_isolated_results_instead_of_throwing_batch(self):
        self.assertIn('DataClient.loadMany(["etf-performance", "fixed-income-performance"])', DATA_LAB_JS)
        self.assertNotIn('DataClient.loadMany(["etf-performance", "fixed-income-performance"])', APP_JS)
    ...
    def test_data_lab_exposes_provenance_and_freshness(self):
        for label in (
            ...
            self.assertIn(label, DATA_LAB_JS)
