...
class UpdateFedPolicyTests(unittest.TestCase):
    def test_fred_query_requests_only_recent_observations(self) -> None:
        csv_payload = b"observation_date,DFEDTARL\n2026-08-04,3.5\n"
        with patch.object(FED_POLICY, "fetch_bytes", return_value=csv_payload) as fetch:
            observation_date, value = FED_POLICY.fred_latest("DFEDTARL")
        requested_url = fetch.call_args.args[0]
        self.assertIn("id=DFEDTARL", requested_url)
        self.assertIn("&cosd=", requested_url)
        self.assertEqual(observation_date.isoformat(), "2026-08-04")
